...
import detectron.datasets.smd_dataset_evaluator as smd_dataset_evaluator
...
def evaluate_boxes(dataset, all_boxes, output_dir, use_matlab=False):
    ...
    elif _use_smd_evaluator(dataset):
        print("Hi I'm your detections evaluator! Please hold the line :)")
        smd_dataset_evaluator.evaluate_boxes(dataset, all_boxes, output_dir)
        smd_dataset_evaluator.draw_histos(filepath=output_dir)
        print("I'm done - have a good day! :)")
        box_results = _smd_eval_to_box_results(['empty result'])
    else:
...
def evaluate_masks(dataset, all_boxes, all_segms, output_dir):
    ...
    if _use_json_dataset_evaluator(dataset):
        ...
        print("segmentations will not be evaluated at the moment! Feel free to implement an evaluation for that.")
        mask_results = _smd_eval_to_mask_results(["empty results"])
    else:
        raise NotImplementedError(
            'No evaluator for dataset: {}'.format(dataset.name)
        )
...
def _use_smd_evaluator(dataset):
    """Check if the datsaset uese the SMD dataset evaluator"""
    return dataset.name.startswith('smd_')
...
def _smd_eval_to_box_results(smd_eval):
    # Not supported (return empty results)
    return _empty_box_results()

def _smd_eval_to_mask_results(smd_eval):
    # Not supported (return empty results)
    return _empty_box_results()
